...
def get_raw_path(args:dict, raw_dir: Path):
    # dataset_raw
    args["dataset_raw"] = raw_dir / args["problem"] / args["dataset_raw"]
    if not args["dataset_raw"].exists():
        raise FileNotFoundError(f"{args['dataset_raw']} not found")
    
def make_prep_path(args:dict, prep_dir: Path):
    # dataset_prep
    if args["dataset_prep"] is None:
        args["dataset_prep"] = args["dataset_raw"].name + " inputs_" + args["inputs"] + " outputs_" + args["outputs"]
        if "n" in args["inputs"]:
            args["dataset_prep"] += " " + args["allin1_prepro_n_case"]
            
    args["dataset_prep"] = prep_dir / args["problem"] / args["dataset_prep"]
    args["dataset_prep"].mkdir(parents=True, exist_ok=True)
    (args["dataset_prep"] / "Inputs").mkdir(parents=True, exist_ok=True)
    (args["dataset_prep"] / "Labels").mkdir(parents=True, exist_ok=True)

def make_model_and_destination_paths(args:dict, models_dir: Path):
    # model, destination
    if args["destination"] is None:
        args["destination"] = args["dataset_prep"].name + " box"+str(args["len_box"]) + " skip"+str(args["skip_per_dir"])
    if args["case"] == "train":
